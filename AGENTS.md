...
- `run-all.ps1 기준으로 Tomcat restart + MigrateBatch all + Capture preset all + Doc Sync까지 수행하고 결과 로그를 보고해줘.`
...
- 자동 주석 파일 인코딩은 UTF-8(무BOM)으로 강제하며, 깨짐 발생 시 자동화 스크립트 기준으로 즉시 재생성한다.
...
- `GET /rays/ui/` -> 200 (`index.html`)
- `GET /rays/ui/static/js/*.js` -> 200
- `GET /rays/ui/static/css/*.css` -> 200
- `GET /rays/ui/<deep-route>` -> 200 (`index.html`)
- 로그인 세션 기준 API 계약 검증:
  - `POST /rays/user/v1/policyCheck` 결과 `resultCode=0`
  - 동일 세션으로 `POST /rays/user/v1/sessionAlive` 결과 `resultCode=0`
  - 동일 세션으로 `POST /rays/user/v1/sessionInfo` 결과 `resultCode=0`
  - `sessionInfo.sessionData.siteCode/levelCode/userId` 필수값 존재
- Browser Console에 아래 에러가 없어야 함:
  - `<Router basename="/ui"> ... URL "/rays/ui" ... won't render`
